...
def setup_logger_kwargs(exp_name, seed=None, data_dir=None, datestamp=False):
    ...
    Args:
        ...
        data_dir (string): Path to folder where results should be saved.
            Default is the ``DEFAULT_DATA_DIR`` in ``playground/user_config.py``.
...
def call_experiment(
    ...
    There's also a RL Playground specific convenience added into executing the
    ...
    Args:
        ...
        data_dir (string): Used in configuring the logger, to decide where
            ...
            default to ``DEFAULT_DATA_DIR`` from ``playground/user_config.py``.
    ...
    plot_cmd = "python3 -m playground.run plot " + logger_kwargs["output_dir"]
    plot_cmd = colorize(plot_cmd, "green")

    test_cmd = "python3 -m playground.run test_policy " + logger_kwargs["output_dir"]
...
class ExperimentGrid:
    ...
    def add(self, key, vals, shorthand=None, in_name=False):
        ...
        ``playground/user_config.py`` file to ``False``.
