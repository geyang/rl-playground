...
def maze_encoder(in_channels):
    encoder = nn.Sequential(
        nn_init(nn.Conv2d(in_channels, 16, kernel_size=8, stride=2), w_scale=np.sqrt(2)),
        nn.ReLU(True),
        nn_init(nn.Conv2d(16, 32, kernel_size=4, stride=1), w_scale=np.sqrt(2)),
        nn.ReLU(True),
    )
    return encoder
