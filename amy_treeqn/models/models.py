...
from amy_treeqn.models.encoding import atari_encoder, push_encoder, maze_encoder
...
class TreeQNPolicy(nn.Module):
    def __init__(self,
        ...
        if input_mode == "atari":
            ...
            dummy = torch.zeros(1, *ob_shape[1:])
            ...
            dummy = torch.zeros(1, *ob_shape[1:])
            conv_dim_out = tuple(encoder(dummy).size())[1:]
        elif input_mode == "maze":
            encoder = maze_encoder(ob_shape[1])
            dummy = torch.zeros(2, *ob_shape[1:])
            conv_dim_out = tuple(encoder(dummy).size())[1:]
            self.obs_scale = 255.0
        else:
    ...
    def obs_to_variable(self, ob, volatile=False):
        ...
        ob = torch.Tensor(ob / self.obs_scale).type(dtype)
        return ob
