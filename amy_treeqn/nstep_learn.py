...
class Learner(object):
    ...
    def train(self, obs, next_obs, returns, rewards, masks, actions, values):
        ...
        action_sequences, reward_sequences, sequence_mask = build_sequences(
            [torch.from_numpy(actions), torch.from_numpy(rewards)], masks, self.nenvs, self.nsteps, self.tree_depth,
            return_mask=True)
        ...
        if self.use_st_loss:
            ...
            st_targets, st_mask = build_sequences([st_embeddings.data], masks, self.nenvs, self.nsteps, self.tree_depth,
                                                  return_mask=True, offset=1)
        ...
        if self.use_subtree_loss:
            ...
            subtree_loss = [(s_taken - s_target).pow(2).mean() for (s_taken, s_target) in
                            zip(subtree_taken_clip, target_subtrees_clip)]
