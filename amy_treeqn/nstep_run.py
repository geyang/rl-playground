...
import numpy as np
from amy_treeqn.envs.maze_wrapper import MazeWrapper
from amy_treeqn.utils.bench.monitor import load_global_results
from amy_treeqn.utils.logger import Logger, HumanOutputFormat
from amy_treeqn.utils.bl_common import explained_variance
from amy_treeqn.utils.seed import set_global_seeds
from amy_treeqn.utils import bench
from amy_treeqn.utils.bl_common.vec_env.subproc_vec_env import SubprocVecEnv
from amy_treeqn.utils.bl_common.atari_wrappers import wrap_deepmind
from amy_treeqn.utils.treeqn_utils import get_timestamped_dir, append_scalar, append_list
from amy_treeqn.models.models import DQNPolicy, TreeQNPolicy
# from treeqn.envs.push import Push
from amy_treeqn.nstep_learn import Learner, Runner
...
def my_config(save_folder, env_id, architecture, label, name):
    ...
    obs_dtype, input_mode = 'uint8', "maze"
    if "push" in env_id:
    ...
    # Create monitor dir
    monitor_dir = os.path.join(save_folder, f"{env_id}, {label}, {name}")
    if not os.path.exists(monitor_dir):
        os.makedirs(monitor_dir)


class Box2Grid(gym.Wrapper):
    def __init__(self, env, n=3, act_scale=1.0):
        super().__init__(env)
        high = env.action_space.high
        low = env.action_space.low
        self.a_dict = np.stack([low, np.zeros_like(low), high]).T * act_scale
        # self.action_space = [gym.spaces.Discrete(n) for _ in range(len(low))]
    ...
    def step(self, act):
        act = [act // 3, act % 3]
        a = [a_d[int(a_i)] for a_d, a_i in zip(self.a_dict, act)]
        ts = self.env.step(a)
        # DMControl timesteps are mot mutable, but observations are.
        ts[0]['a_cont'] = a
        return ts
    ...
    def make_env(rank, dir):
        def _thunk():
            if "push" in env_id:
                ...
                # env = MazeWrapper(gym.make(env_id))
                env = TimeLimit(MazeWrapper(Box2Grid(gym.make(env_id))), max_episode_steps=100)
            env.seed(seed + rank)
            filename = dir and os.path.join(dir, f"{rank}.monitor.json")
            env = bench.Monitor(env, filename)
