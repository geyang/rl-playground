...
def parse_and_execute_grid_search(cmd, args):
    ...
    # to RL Playground implementations, and may be hard to maintain.
...
if __name__ == "__main__":
    ...
    assert (
        cmd in valid_cmds
    ), "Select an algorithm or utility which is implemented in RL Playground."
    ...
    if cmd in valid_help:
        ...
        help_msg = (
            dedent(
                """
            Experiment in RL Playground from the command line with

            \tpython -m playground.run CMD [ARGS...]
        ...
        special_info = (
            ...
            \tpython -m playground.run ppo --env HalfCheetah-v2 --clip_ratio 0.1

            If you need a quick refresher on valid kwargs, get the docstring
            with

            \tpython -m playground.run [algo] --help
